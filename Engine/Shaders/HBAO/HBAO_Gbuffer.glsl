...
void main()
{
    ...
    if (u_Animated)
    {
        ...
        vec4 ViewPos = view * model * localPosition;
        Output.ViewPos = ViewPos.xyz;
        Output.ViewNormal = mat3(boneTransform) * a_Normal;
        //v_EntityID = a_EntityID;
        gl_Position = u_ViewProjection *  model * localPosition;
    }
    else
    {
        Output.ViewPos = vec3(view * model * vec4(a_Pos, 1.0));
        Output.ViewNormal =normalize(mat3(transpose(inverse(view * model))) * a_Normal);
        //v_EntityID = a_EntityID;

        gl_Position = u_ViewProjection *  model * vec4(a_Pos, 1.0);
    }
}
...
vec3 getNormalFromMap()
{
    ...
    vec3 Q1 = dFdx(Input.ViewPos);
    vec3 Q2 = dFdy(Input.ViewPos);
...
void main()
{
    ...
    Position_ = vec4(Input.ViewPos, LinearizeDepth(gl_FragCoord.z));
